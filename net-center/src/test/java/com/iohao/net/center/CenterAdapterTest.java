...
/**
 * Tests center Aeron fragment adapter dispatch safety.
 *
 * @author 渔民小镇
 * @date 2026-05-03
 * @since 25.4
 */
class CenterAdapterTest {
    ...
    void registeredTemplateDispatchesToFragment() {
        ...
        newAdapter().onFragment(newHeaderBuffer(REGISTERED_TEMPLATE_ID), 0, MessageHeaderEncoder.ENCODED_LENGTH, null);
    ...
    void unregisteredTemplateDoesNotEscapeAdapter() {
        ...
        assertDoesNotThrow(() -> newAdapter().onFragment(
    ...
    private static CenterAdapter newAdapter() {
        var setting = CenterServerSetting.builder().build();
        return new CenterAdapter(setting);
    }
